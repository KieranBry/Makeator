...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total makeator balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zMKTRsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zMKTR denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zMKTR denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zMKTR denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zMKTR denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zMKTR denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zMKTR denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zMKTR denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zMKTR denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zMKTR denominations\n"
            "  }\n"

            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in makeator/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in makeator/kb\n"
    ...
    UniValue zmktrObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zmktrObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zmktrObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zMKTRsupply", zmktrObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"makeatoraddress\"\n"
            "\nReturn information about the given makeator address.\n"
            "\nArguments:\n"
            "1. \"makeatoraddress\"     (string, required) The makeator address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"makeatoraddress\", (string) The makeator address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Makeator address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are makeator addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) makeator address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"makeatoraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"makeatoraddress\"  (string, required) The makeator address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    ...
    bool nStaking = false;
    printf("mapHashedBlocks.size = %d\n", mapHashedBlocks.size());
    printf("chainActive.Tip()->nHeight = %d\n", chainActive.Tip()->nHeight);
    if (mapHashedBlocks.count(chainActive.Tip()->nHeight)) {
        printf("mapHashedBlocks.count(chainActive.Tip()->nHeight)\n");        
        nStaking = true;
    }
    else if (mapHashedBlocks.count(chainActive.Tip()->nHeight - 1) && nLastCoinStakeSearchInterval) {
        printf("mapHashedBlocks.count(chainActive.Tip()->nHeight - 1) && nLastCoinStakeSearchInterval\n");        
