...
#if defined(NDEBUG)
#error "Makeator cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umktr) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Makeator Signed Message:\n";
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zMktr with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMktr spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zMKTR spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 1 * COIN;
    } else if (nHeight > 200 && nHeight <= 20000) {
        nSubsidy = 2 * COIN;
    } else if (nHeight > 20001 && nHeight <= 80000) {
        nSubsidy = 30 * COIN;
    } else if (nHeight > 80001 && nHeight <= 250000) {
        nSubsidy = 20 * COIN;
    } else if (nHeight > 250001 && nHeight <= 703890) {
        nSubsidy = 10 * COIN;
    } else if (nHeight > 703891 && nHeight <= 1104920) {
        nSubsidy = 5 * COIN;
    } else if (nHeight > 1104921 && nHeight <= 1480001) {
        nSubsidy = 2 * COIN;
    } else {
        nSubsidy = 2 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Makeator
...
{
    RenameThread("makeator-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMKTRMinted()
{
...
void RecalculateZMKTRSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMKTR supply
...
bool RecalculateMKTRSupply(int nHeightStart)
{
...
bool UpdateZMKTRSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (Params().Zerocoin_Block_RecalculateAccumulators() >= 0 && pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateMKTRSupply(Params().Zerocoin_StartHeight());
    }

    //Track zMKTR money supply in the block index
    if (!UpdateZMKTRSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zMKTR supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMktrSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MKTR to zMKTR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMktr spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMktr serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMktr mints and %d zMktr spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Makeator: We use certain sporks during IBD, so check to see if they are
        // available. If not, ask the first peer connected for them.
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_14_NEW_PROTOCOL_ENFORCEMENT) &&
                !pSporkDB->SporkExists(SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2) &&
                !pSporkDB->SporkExists(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) &&
                !pSporkDB->SporkExists(SPORK_17_NEW_PROTOCOL_TARGETTIME);
