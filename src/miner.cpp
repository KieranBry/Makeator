...
//    LogPrintf("CreateNewBlock(): fProofOfStake=%d\n", fProofOfStake);
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zmktrs that have been in mempool long
                    //and higher priority to zmktrs that are large in value
                    ...
                    // zMKTR spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zMktr spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zMktr serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MakeatorMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MakeatorMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    LogPrintf("MakeatorMiner started with fProofOfStake=%d\n", fProofOfStake);
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("makeator-miner");
    ...
    if (fProofOfStake && (GetTime() - nMintableLastCheck > 5 * 60)) // 5 minute check time
    {
        ...
        if (fTUDebug) printf("fMintableCoins = %d\n", fMintableCoins);
    }

    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            if (fTUDebug) printf("chainActive.Tip()->nHeight = %d\n", chainActive.Tip()->nHeight);
            if (fTUDebug) printf("Params().LAST_POW_BLOCK() = %d\n", Params().LAST_POW_BLOCK());
            ...
            while (chainActive.Tip()->nTime < Params().GenesisBlock().nTime || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || 
               ...
               if (fTUDebug) printf("-----\n");
               if (fTUDebug) printf("chainActive.Tip()->nTime = %d\n", chainActive.Tip()->nTime);
               if (fTUDebug) printf("Params().GenesisBlock().nTime = %d\n", Params().GenesisBlock().nTime);
               if (fTUDebug) printf("vNodes.empty() = %d\n", vNodes.empty());
               if (fTUDebug) printf("pwallet->IsLocked() = %d\n", pwallet->IsLocked());
               if (fTUDebug) printf("!fMintableCoins = %d\n", !fMintableCoins);
               if (fTUDebug) printf("nReserveBalance = %lld\n", nReserveBalance);
               if (fTUDebug) printf("pwallet->GetBalance() = %lld\n", pwallet->GetBalance());
               if (fTUDebug) printf("!masternodeSync.IsSynced() = %d\n", !masternodeSync.IsSynced());
            ...
            if (fTUDebug) printf("check mapHashedBlocks\n");
        ...
        if (fTUDebug) printf("creating new block\n");
        ...
        LogPrintf("Running MakeatorMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
